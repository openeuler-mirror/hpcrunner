...
REG_META_HVALUE=$(md5sum ${JARVIS_DOWNLOAD}/lapack-3.8.0.tgz | awk '{print $1}')
REG_META_HTYPE="md5"
REG_META_PACKAGE="lapack-3.8.0.tgz"
REG_META_TYPE="tgz"

[ -d lapack-3.8.0 ] && echo "Exist DIR:$(pwd)/lapack-3.8.0" && exit 1
tar -xvf ${JARVIS_DOWNLOAD}/lapack-3.8.0.tgz -C ./
[ ! $? -eq 0 ] && echo "Invalid file: lapack-3.8.0.tgz" && exit 1
cd lapack-3.8.0
REG_PROJECT_URL=$(pwd)
...
cmake .. -DCMAKE_INSTALL_PREFIX="${PATH_INSTALL}" -DBUILD_SHARED_LIBS=ON -DCMAKE_BUILD_TYPE=RELEASE -DCMAKE_INSTALL_LIBDIR="${PATH_INSTALL}/lib64" -DBUILD_TESTING=ON -DBUILD_SHARED_LIBS=ON -DLAPACKE=ON -DLAPACKE_WITH_TMG=ON -DCBLAS=ON -DBUILD_DEPRECATED=ON
make  VERBOSE=1 -j32 
make install -j32
