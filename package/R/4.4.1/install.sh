...
mkdir -p ${WORK_DIR}
{
. ${DOWNLOAD_TOOL} -u https://cran.r-project.org/src/base/R-4/R-4.4.1.tar.gz
cd ${WORK_DIR}
[ -d R-4.4.1  ] && echo "Exist DIR:$(pwd)/R-4.4.1 " && exit 1
REG_META_HVALUE=$(md5sum ${JARVIS_DOWNLOAD}/R-4.4.1.tar.gz | awk '{print $1}')
REG_META_HTYPE="md5"
REG_META_PACKAGE="R-4.4.1.tar.gz"
REG_META_TYPE="tgz"
tar -xvf ${JARVIS_DOWNLOAD}/R-4.4.1.tar.gz
[ ! $? -eq 0 ] && echo "Invalid file: R-4.4.1.tar.gz" && exit 1
cd R-4.4.1
./configure -enable-R-shlib --with-libpng --with-jpeglib --prefix=$1 \
