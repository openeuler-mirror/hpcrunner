...
set -e
hmpi_version='1.3.1'
. ${DOWNLOAD_TOOL} -u https://gitee.com/kunpengcompute/hucx/repository/archive/v1.3.1.spc001-huawei.tar.gz -f hucx-${hmpi_version}-huawei.tar.gz
. ${DOWNLOAD_TOOL} -u https://gitee.com/kunpengcompute/xucg/repository/archive/v1.3.1-huawei.tar.gz -f xucg-${hmpi_version}-huawei.tar.gz
. ${DOWNLOAD_TOOL} -u https://gitee.com/kunpengcompute/hmpi/repository/archive/v1.3.1.spc001-huawei.tar.gz -f hmpi-${hmpi_version}-huawei.tar.gz
cd ${JARVIS_TMP}
. $CHECK_ROOT && yum install -y perl-Data-Dumper autoconf automake libtool binutils flex
rm hucx-v${hmpi_version}.spc001-huawei -rf
tar xf ${JARVIS_DOWNLOAD}/hucx-${hmpi_version}-huawei.tar.gz
cd hucx-v${hmpi_version}.spc001-huawei
./autogen.sh
...
rm hmpi-v${hmpi_version}.spc001-huawei/ -rf
tar xf ${JARVIS_DOWNLOAD}/hmpi-${hmpi_version}-huawei.tar.gz
cd hmpi-v${hmpi_version}.spc001-huawei/
./autogen.pl
