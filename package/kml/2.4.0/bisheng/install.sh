...
set -e
kml_version=2.4.0
# . ${DOWNLOAD_TOOL} -u https://kunpeng-repo.obs.cn-north-4.myhuaweicloud.com/Kunpeng%20BoostKit/Kunpeng%20BoostKit%2023.0.0/BoostKit-kml_${kml_version}_bisheng.zip
. ${DOWNLOAD_TOOL} -u https://kunpeng-repo.obs.cn-north-4.myhuaweicloud.com/Kunpeng%20HPCKit/Kunpeng%20HPCKit%2024.0.RC2/KML_${kml_version}_bisheng.zip
...
rpm -q kml-${kml_version}-1.aarch64 && rpm -e --nodeps --noscripts kml-${kml_version}-1.aarch64 2>/dev/null
...
netlib=${JARVIS_DOWNLOAD}/lapack-3.12.0.tar.gz
...
mkdir build
cd build
cmake -DCMAKE_BUILD_TYPE=Release -DCMAKE_POSITION_INDEPENDENT_CODE=ON -DBUILD_DEPRECATED=ON ../lapack-3.12.0
make -j
cd ../
...
export CFLAGS='-Wno-error=implicit-function-declaration' CXXFLAGS='-Wno-error=implicit-function-declaration'
cd ${JARVIS_TMP}
rm -rf netlib2
mkdir netlib2
cd netlib2
...
clang -o libkscalapack_full.so -shared -fPIC -Wl,--whole-archive $kscalapack libscalapack_adapt.a -Wl,--no-whole-archive $klapack liblapack_adapt.a  $kservice -fopenmp -lpthread -lflang -lm
