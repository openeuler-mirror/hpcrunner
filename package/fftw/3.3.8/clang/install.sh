...
mkdir -p ${JARVIS_DEV_VROOT}/fftw/clang/${BASENAME}
{
. ${DOWNLOAD_TOOL} -u http://www.fftw.org/fftw-3.3.8.tar.gz
cd ${JARVIS_DEV_VROOT}/fftw/clang/${BASENAME}
...
} 2>&1 | tee ${JARVIS_DEV_VROOT}/fftw/clang/${BASENAME}/${log_file}
res=${PIPESTATUS[0]}
echo "Install Result:${res}"
cp ${JARVIS_DEV_VROOT}/fftw/clang/${BASENAME}/${log_file} ${1}
set +x
exit ${res}
