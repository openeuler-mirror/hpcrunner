...
REG_META_HVALUE=$(md5sum ${JARVIS_DOWNLOAD}/proj-5.2.0.tar.gz | awk '{print $1}')
REG_META_HTYPE="md5"
REG_META_PACKAGE="proj-5.2.0.tar.gz"
REG_META_TYPE="tgz"
[ -d proj-5.2.0 ] && echo "Exist DIR:$(pwd)/proj-5.2.0" && exit 1
tar -xvf ${JARVIS_DOWNLOAD}/proj-5.2.0.tar.gz
[ ! $? -eq 0 ] && echo "Invalid file: proj-5.2.0.tar.gz" && exit 1
