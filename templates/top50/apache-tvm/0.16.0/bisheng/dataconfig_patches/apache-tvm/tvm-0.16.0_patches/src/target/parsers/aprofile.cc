...
static TargetFeatures GetFeatures(TargetJSON target) {
  ...
  LOG(WARNING) << "aarch64:" << Bool(IsAArch64(mtriple)) << "asimd:" << Bool(has_feature("neon")) << "sve:" << Bool(has_feature("sve"))
	  << "dotprod" << Bool(has_feature("dotprod")) << "matmul_i8:" << Bool(has_feature("i8mm")) << "fp16_simd:" << Bool(has_feature("fullfp16")) << " has_sme:" << Bool(has_feature("sme"));
