...
echo "start ..."
sleep 5
{ time  mpirun --hostfile $HOSTFILE -np 16 --allow-run-as-root --map-by ppr:1:numa:PE=36 -x OMP_PROC_BIND=close -x OMP_PLACES=cores -x KML_BLAS_THREAD_TYPE=OMP -x KML_BLAS_NOT_USE_HBM=0 -x OMP_NUM_THREADS=36 -x UCX_TLS=sm  -x PATH -x LD_LIBRARY_PATH ./kscalapack_pdgetrf_cpp_kml -nb 512 -grid 1x16 -size 120000;} 2>&1 | tee tee_kscalapack_pdgetrf_cpp_kml.txt
