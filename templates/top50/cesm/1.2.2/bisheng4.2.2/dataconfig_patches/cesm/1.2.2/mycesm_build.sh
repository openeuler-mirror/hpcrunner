...
function build_test()
{
	...
	sed -i "163i\ <MAX_TASKS_PER_NODE>608</MAX_TASKS_PER_NODE>" scripts/ccsm_utils/Machines/config_machines.xml
	...
	sed -i '375i\<ADD_CFLAGS> -O3 -finit-zero -mcpu=hip11 </ADD_CFLAGS>' scripts/ccsm_utils/Machines/config_compilers.xml
	sed -i "378c <FFLAGS> -O3 -march=native -ffast-math -funroll-loops -fstack-arrays -finit-zero -fconvert=big-endian -ffree-line-length-none -ffixed-line-length-none -mcpu=hip11 -I$NETCDF_PATH/include -I$HDF5_PATH/include -I$PNETCDF_PATH/include -I$CURL_PATH/include -I$OPENBLAS_PATH/include -I$ZLIB_PATH/include -fopenmp </FFLAGS>" scripts/ccsm_utils/Machines/config_compilers.xml
	...
	./xmlchange NTASKS_ATM=512,NTASKS_LND=216,NTASKS_ROF=40,NTASKS_ICE=256,NTASKS_OCN=80,NTASKS_GLC=40,NTASKS_WAV=40,NTASKS_CPL=512,ROOTPE_ICE=0,ROOTPE_ATM=0,ROOTPE_LND=256,ROOTPE_ROF=472,ROOTPE_OCN=512,ROOTPE_GLC=472,ROOTPE_WAV=472,ROOTPE_CPL=0
	echo "Case setup ................"
	./cesm_setup
	...
	if patch -p0 --dry-run < "$PATCH_FILE"; then
	  patch -p0 < "$PATCH_FILE"
	  echo "patch OK!!!"
	else
	  echo "patch Failed!!!"
