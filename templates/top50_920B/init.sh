...
# C2 /workp/software
#export JARVIS_SOFT_ROOT=/mnt/sdb/wlj/share/software
# C2 /workp/system
#export JARVIS_SYSTEM_ROOT=/mnt/sdb/wlj/share/system
export JARVIS_SOFT_ROOT=/home/wanlijun/c2root/share/software
export JARVIS_SYSTEM_ROOT=/home/wanlijun/c2root/share/system
export JARVIS_LOG_DIR=${CUR_PATH}/logs
...
if [ "$JARVIS_MODE" -eq 0 ]; then
    ...
    export JARVIS_JOBSCRIPT_LIBSHELL=${CUR_PATH}/package/common/bash-com-kit.sh
    export JARVIS_YUM_SYSTEM=${JARVIS_SYSTEM_ROOT}/openEuler_22.03SP4
    export JARVIS_YUM_CONFIG=${JARVIS_ROOT}/system/openEuler_22.03SP4/etc/yum.repos.d/openEuler.repo
    cp ${JARVIS_ROOT}/templates/top50/system ./ -ar
    echo "init system yum.repo config:$(ls system/openEuler_22.03SP4/etc/yum.repos.d/*.repo)"
    if [ -f ${JARVIS_ROOT}/templates/top50/ddeployment.cfg ]; then
	cp ${JARVIS_ROOT}/templates/top50/deployment.cfg ${JARVIS_ROOT}/deployment.cfg -ar
    fi    
fi
export JARVIS_TMP=/tmp
export JARVIS_DEV=${CUR_PATH}/dev
export JARVIS_DEV_VROOT=${CUR_PATH}/dev/vroot
mkdir -p ${JARVIS_DEV_VROOT}
...
export JARVIS_LIBSHELL=${CUR_PATH}/package/common/libshell.sh

#Install modules
if ! type module >/dev/null 2>&1; then
    echo "Install environment-modules"
    # yum --installroot=${JARVIS_YUM_SYSTEM} install [ksh] --config=/etc/yum.repos.d/openEuler.repo
