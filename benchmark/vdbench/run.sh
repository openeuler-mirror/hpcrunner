...
vdbench_test(){
	if ! type java >/dev/null 2>&1;then
		echo "==================vdbench测试：安装java=================="
		. $CHECK_ROOT && yum install -y java-1.8.0
	fi
	if ! check_file "8k_randread";then
                echo "==================hpcg_test测试:缺少8k_randread文件，请在$JARVIS_ROOT下编写8k_randread=================="
                return 1
        fi
	mkdir -p ${run_dir}
	singularity run -B ${run_dir}:/root --pwd=${run_dir} ${download_dir}/Benchmark24.0.0.sif cp -r /hpcrunner/tools/vdbench ./
	cd ${run_dir}/vdbench
	\cp $JARVIS_ROOT/8k_randread ./
	./vdbench -f   8k_randread
}
